...
public class AddTestCaseServletWeb extends HttpServlet {
...
//		String testID = createWebTestID();
		...
		String sqlOne = "Insert into TestCase values('"+testUID+"','"+testCaseName+"','"+description+"','Web','NA',0,0,0,0,0,0,0,0,'Eminem',0,'Vennommmm','N','email@email.com','VX','"+date+"','"+date+"');";
		...
		try {
			...
			CreatePageActionWeb.createFile(testCaseName, annotation, stepsDesc,operation,locator,locatorDest,validationType,mouseAction,inputText);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}	
